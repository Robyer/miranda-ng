...
INT_PTR CALLBACK FBOptionsMessagingProc(HWND hwnd, UINT message, WPARAM wparam, LPARAM lparam)
{
	...
	case WM_INITDIALOG:
	{
		...
		LoadDBCheckState(proto, hwnd, IDC_AUTO_ACCEPT_MESSAGES, FACEBOOK_KEY_AUTO_ACCEPT_MESSAGES, DEFAULT_AUTO_ACCEPT_MESSAGES);
	...
	case WM_NOTIFY:
	{
		if (reinterpret_cast<NMHDR*>(lparam)->code == PSN_APPLY)
		{
			...
			StoreDBCheckState(proto, hwnd, IDC_AUTO_ACCEPT_MESSAGES, FACEBOOK_KEY_AUTO_ACCEPT_MESSAGES);
